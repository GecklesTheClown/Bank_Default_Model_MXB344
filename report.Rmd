...
We done violin plots between our discrete variable (employment length) and the three continuous variables to examine the relationship between the discrete predictor and the different continuous predictors.  
...
Plotted the proportions of repay failure against our 4 categorical variables and 1 of our discrete variables (that has been categorized). 
...
We plotted violin plots between our discrete variable (employment length) and the three continuous variables to examine the relationship between the discrete predictor and the different continuous predictors.  
...
```
We can see that there still exists a rough positive linear relationship between the logit of the repay failure and loan amount for most of the address states.
...
```
We can see that there still exists a strong positive linear relationship between the logit of the repay failure and interest rate for most of the address states.
...
```
We can see that there still exists a moderate negative linear relationship between the logit of the repay failure and annual incrome for most of the address states.
...
```
We can see that there still exists a rough positive linear relationship between the logit of the repay failure and loan amount for most of the issue dates.
...
```
We can see that there still exists a strong positive linear relationship between the logit of the repay failure and interest rate for most of the issue dates.
...
```
We can see that there still exists a moderate negative linear relationship between the logit of the repay failure and annual income for most of the issue dates.

In conclusion we will still consider doing a logistic regression when accounting for the time and location random effects. Thus we will consider doing a logistic regression for the generalised linear mixed effects model (GLMM).
...
summary(m)
predicted_values.m <- predict(m, type = 'response', newdata = extended_loan_data)
...
plot_summs(forward_sel_model,m, rescale.distributions = TRUE)
```

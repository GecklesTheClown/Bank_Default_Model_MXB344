...
roc.glmm(Final_glmm)
```
We can see that the glmm with the random effect zip code perfroms better compared to addr state
...
plot_summs(final_glm,Final_glmm, rescale.distributions = TRUE,exp = TRUE,omit.coefs = )

#"emp_length10+ years""emp_lengthn/a""purposedebt_consolidation""purposeeducational""purposemedical""purposemoving""purposeother"
#"purposesmall_business""home_ownershipOTHER""annual_inc""int_rate:annual_inc")
```
...
We can see from the plots above which shows the estimated coefficient and corresponding confidence intervals for the random effects variables. We can see from the issue_id plot that confidence interval for the random effects at the tips do not pass overlapped zero which suggest that does values are significantly different from zero. This would suggest that issue date (time) has a significant impact on repay failure at 95% CI. From the plot of the addr_state we can that see a couple of the confidence intervals at the top of the tail do not overlapp zero which would suggest that those coefficients a significantly greater than zero. However, the vast majority of random effects overlapped zero suggesting that state effect on repay failure is insignificant. This would probably require further investigation by assessing the random effect of zip_code.

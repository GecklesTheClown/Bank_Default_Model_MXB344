...
This is good but let's see if step-wise regression to make it better. Different variations of stepwise regression were used (both, backward & forward)
...
It seems all model agree and thus have all converged to the same model. Thus a random one will be picked.

## Model Selection
```{r, model-per message=FALSE, cache=TRUE}
summary(both_sel_model)
roc.glm(both_sel_model)

```
It can be seen that emp_length may not be need so a Chi-squared test was conducted to see.
...
```
It looks like it is needed as the p-value is < 0.05. Next using information from the exploratory analysis section some extra model testing was conducted. This testing will be in the hopes of optimizing AUC value.
...
The final_glm was found to have the best auc value but it has few terms which were not significant, using the same methodology as before, nested models will be tested to see which terms are neccessary.
...
roc.glm(final_model_nested <- nested_final_5)

```
Let's check this was helpful will BIC aswell.
...
final_model_nested_cloglog <- glm(formula = repay_fail ~
             int_rate + 
             term + 
             emp_length + 
             purpose + 
...
roc.glm(final_glm_logit) ; roc.glm(final_glm_probit) ; roc.glm(final_glm_cloglog)

```
It appears that keeping it with the canonical logit link function produces the best AUC values. Thus our final glm model is:
```{r}
summary(final_model_nested_logit)
roc.glm(final_model_nested_logit)
plot_summs(final_model_nested_logit)
```

Converting the Gini value given in the task of the report would give an AUC value of 0.11 (old model). Our model is significantly better for both the training data and validation data.
...
Employment/a years (mildly significant):Having a employmentlength/a, versus a employment length of less than 1 year, changes the odds of repay failure by a factor of 1.79
...
Using our knowledge from the fixed effects model we can test some fixed effects models. We have decided to use our model with the best AUC value instead of our nested model because we do not know how terms will change in significance after adding random effects.
...
```
Now these two models will be compared using AUC values.
...
Both our fixed-effects and mixed effects model perform significantly better in terms of (AUC values) to the model used prior of the acquisition.

2. What are the important variables in this model?

It vital for banks to be able explain how an application is assessed, this coincides with our models proposed significant variables. Talking in terms of the fixed effects model, these include: int_rate, term, emp_length, purpose, verification_statu annual_inc

3. Can accounting for this variation (e.g., state/zip-code and time) improve performance benchmarks?

Indeed yes our AUC value went from 0.6910845 to ______ this is a marginal improvement

4. Are there any surprising differences in variables that are important for predicting credit risk, between
your model with/without location and time information?

One surprising difference found between the two models was the significance of verification_status in our fixed effects model this was significant while in our random effects model it was not

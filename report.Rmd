...
After this short explanation of the dataset we will do some initial exploration.
...
full_interaction_model2 <- glm(data = all_loan_data,
                              formula = repay_fail ~ .^2 ,
                              family = binomial)
...
both_sel_model <- backward_sel_model <- stepAIC(full_interaction_model, 
                              direction = "both",
                              trace = 0) 

# both_sel_model2 <- backward_sel_model <- stepAIC(full_interaction_model2, 
#                               direction = "both",
#                               trace = 0) 


m1<-glm(formula = repay_fail ~ 
          int_rate + 
          annual_inc + 
          term, 
...
roc.glm(forward_sel_model) ; roc.glm(m1) ; roc.glm(m.step.both.1w.clog)
...
```{r}
plot_summs(forward_sel_model, ci_level = 0.95,plot.distributions = TRUE)
```
...
```
## Initial Model as seen in ppt

```{r}
test2 <- glm(formula = repay_fail ~ 
               term + 
               purpose +                                
               verification_status +  
               annual_inc,

...
```{r data, include=TRUE}
All_loan_data <- read.csv(file = "all_loan_data.csv") # Load in the loan data

Ext_loan_data <- read.csv(file = "extended_loan_data.csv")  # Load in the extended loan data with time and regions

## There are variables that we won't study because they weren't available at the time of the loan application, these varaibles are,
# "credit_age_yrs"
# "delinq_2yrs"
# "dti"
# "inq_last_6mths"
...
# Thus in the All_loan_data this leaves use with 5 catagorical variables and 3 continous varaibles
...
All_loan_data$term <- factor(All_loan_data$term)
levels(All_loan_data$term)
All_loan_data$emp_length <- factor(All_loan_data$emp_length)
levels(All_loan_data$emp_length)
All_loan_data$home_ownership <- factor(All_loan_data$home_ownership)
levels(All_loan_data$home_ownership)
All_loan_data$verification_status <- factor(All_loan_data$verification_status)
levels(All_loan_data$verification_status)
All_loan_data$purpose <- factor(All_loan_data$purpose)
levels(All_loan_data$purpose)
```
